...
import { getEntity, updateEntity, createEntity, reset, getEntities, deleteEntity } from './ods.reducer';
...
const OdsCRUD = (props: RouteComponentProps<{ id: string; index: string }>) => {
  ...
  const [filters, setFilters] = useState({
    ...
    ods: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
  });
  ...
  const rightToolbarTemplate = () => {
    return (
      <React.Fragment>
        <Button label="Nuevo Ods" icon="pi pi-plus" className="p-button-info" onClick={verDialogNuevo} />
      </React.Fragment>
    );
  };
    ...
    <div className="grid">
      <div className="col">
        <div className="flex justify-content-start  font-bold  m-2">
          <div className="text-primary text-xl">Ods</div>
        </div>
      </div>
      <div className="col">
      ...
      <div className="col-12">
        <div className="card">
          ...
          <DataTable
            ...
            currentPageReportTemplate="Mostrando del {first} al {last} de {totalRecords} Ods"
            filters={filters as DataTableFilterMeta}
            filterDisplay="menu"
            loading={loading}
            emptyMessage="No hay Tipo de Ods.."
            header={header}
            responsiveLayout="stack"
          >
            <Column field="id" header="Id" hidden></Column>
            <Column field="ods" header="ODS" sortable headerStyle={{ minWidth: '50rem' }}></Column>

            <Column body={actionBodyTemplate} headerStyle={{ minWidth: '10rem' }}></Column>
          </DataTable>

          <Dialog visible={retoDialogNew} style={{ width: '450px' }} header="Ods" modal className="p-fluid  " onHide={hideDialogNuevo}>
          ...
          >
            <div className="flex align-items-center justify-content-center">
              ...
              {selectedTipoIdea && (
                <span>
                  ¿Seguro que quiere eliminar el Ods: <b>{selectedTipoIdea.ods}</b>?
                </span>
              )}
            </div>
          </Dialog>
